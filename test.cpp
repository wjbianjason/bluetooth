...
{
	fstream fin,fout;
	fin.open("id-id-predicates.txt",ios::in);
	...
	while(!fin.eof())
	{
		...
		class1_id = entity_class[entity1_id]; 
		class2_id = entity_class[entity2_id];
		pre_name = pre_id_name[predicate_id];
		bool flag = true;
		if(class1_id>class2_id)		//swap ids of class1 and class2
		{
	 	 	class2_id = class1_id + class2_id;
			class1_id = class2_id - class1_id;
			class2_id = class2_id - class1_id;
			flag = false;
		}
		int hash_code = class1_id * 1000 + class2_id;
...
/**
* [similar_check is used to check the semantic similarity of predicate1’s name and predicate2’s name]		
...
bool similar_check(string pre1,string pre2)
{
	...
	father1.clear();		
	father2.clear();	
	if(it1->second->name->father_name.size()!=0)	//if pre1 has fathers		
	{
		find_father(it1->second->name->father_name,father1);// find_father() uses father1 to store pre1's fathers
	  	if(it2->second->name->father_name.size()!=0)	//if pre2 has fathers
